...
{
    public static void MockRepository<T>(this AutoMock autoMock, T returnObject) where T : class
    {
        autoMock.Mock<ICmdRepository<T>>()
                .Setup(x => x.InsertAsync(It.IsAny<T>(), false)).ReturnsAsync(returnObject);
